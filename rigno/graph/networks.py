...
class InteractionNetworkLayer(MessagePassing):
  ...
  def __init__(self, node_latent_size, edge_latent_size, mlp_hidden_layers=1,
               conditioned_normalization=True, cond_norm_hidden_size=16):
    super().__init__(aggr="mean", flow="source_to_target")
    kwargs = dict(use_layer_norm=True, use_conditional_norm=conditioned_normalization,
  ...
  def forward(self, sender, receiver, edge_index, edge_attr, condition=None):
    ...
    for batch_index in range(sender.shape[0]):
      ...
      delta = self.edge_mlp(edge_attr, sender[batch_index, source],
                            receiver[batch_index, target], condition=cond)
      updated_edges = edge_attr + delta
      aggregated = self.propagate(edge_index=edge_index, edge_attr=updated_edges,
