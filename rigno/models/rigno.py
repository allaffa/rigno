...
class RegionInteractionGraphSet:
  ...
  r2p: TypedGraph
  def to(self, device):
    return RegionInteractionGraphSet(self.p2r.to(device), self.r2r.to(device), self.r2p.to(device))

class RegionInteractionGraphBuilder:
  """Construct physical/regional multiscale graphs using NumPy and SciPy."""
  _domain_shifts = np.array([[0, 0], [-2, 0], [-2, 2], [0, 2], [2, 2],
                             [2, 0], [2, -2], [0, -2], [-2, -2]], dtype=np.float32)
  def __init__(self, periodic, rmesh_levels, subsample_factor, overlap_factor_p2r,
               overlap_factor_r2p, node_coordinate_freqs):
  ...
  def _r2r_edges(self, x):
    ...
    for level in range(self.rmesh_levels):
      ...
      if size < 4:
        continue
      points = x[:size]
      extended = (points[None] + self._domain_shifts[:, None]).reshape(-1, 2) if self.periodic else points
  ...
  def build_metadata(self, x_inp, x_out, domain, rmesh_correction_dsf=1, seed=0):
    x_inp, x_out, domain = map(lambda v: np.asarray(v, dtype=np.float32), (x_inp, x_out, domain))
    ...
    if rmesh_correction_dsf > 1:
      x_rnodes = x_rnodes[rng.permutation(len(x_rnodes))[:int(len(x_rnodes) / rmesh_correction_dsf)]]
    elif rmesh_correction_dsf < 1:
      x_rnodes = _upsample(x_rnodes, 1 / rmesh_correction_dsf, rng)
    radii = self._support_radii(x_rnodes)
    p2r = self._support_edges(x_rnodes, x_inp, np.clip(self.overlap_factor_p2r * radii, 0, radii.max()))
    r2r, domains = self._r2r_edges(x_rnodes)
    r2p = np.flip(self._support_edges(x_rnodes, x_out,
      np.clip(self.overlap_factor_r2p * radii, 0, radii.max())), axis=-1)
  ...
  def _node_features(self, x, radius=None):
    if self.periodic:
      ...
      features = np.concatenate([np.sin(phi[..., None] * freqs).reshape(len(x), -1),
                                 np.cos(phi[..., None] * freqs).reshape(len(x), -1)], axis=-1)
    else:
      features = x
  ...
  def _edge_set(self, sender_name, receiver_name, x_sender, x_receiver, indices, domains=None):
    ...
    if self.periodic:
      if domains is None:
        ...
        rel = x_sender[source] + self._domain_shifts[domains[:, 0]] - x_receiver[target] - self._domain_shifts[domains[:, 1]]
    length = 2 * math.sqrt(x_sender.shape[-1])
    features = np.concatenate([rel / length, np.linalg.norm(rel, axis=-1, keepdims=True) / length], axis=-1)
    return EdgeSet(torch.as_tensor(indices.T.copy(), dtype=torch.long),
                   torch.as_tensor(features, dtype=torch.float32), sender_name, receiver_name)

  def build_graphs(self, metadata):
    xp, xo, xr, rr = metadata.x_pnodes_inp, metadata.x_pnodes_out, metadata.x_rnodes, metadata.r_rnodes
    pnodes_in, pnodes_out, rnodes = self._node_features(xp), self._node_features(xo), self._node_features(xr, rr)
    return RegionInteractionGraphSet(
      TypedGraph({'pnodes': pnodes_in, 'rnodes': rnodes},
                 {'p2r': self._edge_set('pnodes', 'rnodes', xp, xr, metadata.p2r_edge_indices)}),
      TypedGraph({'rnodes': rnodes},
                 {'r2r': self._edge_set('rnodes', 'rnodes', xr, xr, metadata.r2r_edge_indices, metadata.r2r_edge_domains)}),
      TypedGraph({'rnodes': rnodes, 'pnodes': pnodes_out},
...
class RIGNO(AbstractOperator):
  ...
  def __init__(self, num_outputs, processor_steps=18, node_latent_size=128,
               edge_latent_size=128, mlp_hidden_layers=1, concatenate_t=True,
               concatenate_tau=True, conditioned_normalization=True,
               cond_norm_hidden_size=16, p_edge_masking=.5):
    super().__init__()
    self.num_outputs, self.concatenate_t, self.concatenate_tau = num_outputs, concatenate_t, concatenate_tau
    self.conditioned_normalization, self.p_edge_masking = conditioned_normalization, p_edge_masking
    common = (node_latent_size, edge_latent_size, mlp_hidden_layers, conditioned_normalization, cond_norm_hidden_size)
    ...
    self.output_projection = FeedForwardBlock([node_latent_size] * mlp_hidden_layers + [num_outputs])
    self._configs = dict(num_outputs=num_outputs, processor_steps=processor_steps,
      node_latent_size=node_latent_size, edge_latent_size=edge_latent_size,
      mlp_hidden_layers=mlp_hidden_layers, concatenate_t=concatenate_t,
      concatenate_tau=concatenate_tau, conditioned_normalization=conditioned_normalization,
      cond_norm_hidden_size=cond_norm_hidden_size, p_edge_masking=p_edge_masking)
    self.intermediates = {}
  ...
  def forward(self, inputs: Inputs, graphs: RegionInteractionGraphSet):
    ...
    t, tau = self._channel(inputs.t, batch, device, dtype), self._channel(inputs.tau, batch, device, dtype)
    forced = []
    if self.concatenate_t:
      if t is None: raise ValueError("t is required when concatenate_t=True")
      forced.append(t[:, None].expand(-1, points, -1))
    if self.concatenate_tau:
      if tau is None: raise ValueError("tau is required when concatenate_tau=True")
    ...
    edge = graphs.p2r.edge_by_name('p2r'); edge_index, edge_attr = self._masked(edge)
    latent_p, latent_r = self.encoder(torch.cat([features, p_struct], -1), r_struct,
                                      edge_index, edge_attr.to(dtype), condition)
    encoded_r = latent_r
    edge = graphs.r2r.edge_by_name('r2r'); edge_index, edge_attr = self._masked(edge)
    latent_r = self.processor(latent_r, edge_index, edge_attr.to(dtype), condition)
    edge = graphs.r2p.edge_by_name('r2p'); edge_index, edge_attr = self._masked(edge)
    out_count = graphs.r2p.node_features['pnodes'].shape[0]
    receiver = latent_p if latent_p.shape[1] == out_count else torch.zeros(
      batch, out_count, latent_p.shape[-1], device=device, dtype=dtype)
    _, decoded = self.decoder(latent_r, receiver, edge_index, edge_attr.to(dtype), condition)
    self.intermediates = {'pnodes_encoded': latent_p, 'rnodes_encoded': encoded_r,
                          'rnodes_processed': latent_r, 'pnodes_decoded': decoded}
...
def _compute_triangulation_medians(tri):
  ...
  for i in range(tri.simplices.shape[1]):
    left, right = [part.squeeze(1) for part in np.split(tri.points[np.delete(tri.simplices, i, axis=1)], 2, axis=1)]
    edges[:, i] = np.linalg.norm(left - right, axis=1)
  for i in range(tri.simplices.shape[1]):
    value = (2 * np.sum(np.delete(edges, i, axis=1) ** 2, axis=1) - edges[:, i] ** 2) / 4
    medians[:, i] = .67 * np.sqrt(np.maximum(value, 0))
  return medians
