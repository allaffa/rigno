...
def test_message_passing_matches_manual_mean_aggregation():
  ...
  layer = InteractionNetworkLayer(5, 4, mlp_hidden_layers=0,
...
def test_message_passing_is_permutation_equivariant():
  ...
  permuted_edges = torch.stack([
    sender_inverse[edge_index[0]], receiver_inverse[edge_index[1]]])
  actual, actual_edges = layer(sender[:, sender_perm], receiver[:, receiver_perm],
                               permuted_edges, edge_attr)
...
def test_samples_in_a_batch_do_not_interact():
  ...
  for index in range(sender.shape[0]):
    nodes, edges = layer(sender[index:index + 1], receiver[index:index + 1],
