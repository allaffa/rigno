...
def test_rigno_forward_and_backward():
    ...
    output = model(
        Inputs(u=u, c=None, x_inp=x, x_out=x, t=torch.zeros(3, 1), tau=torch.full((3, 1), 0.1)),
        graphs,
    )
    ...
    assert all(
        gradient is not None and torch.isfinite(gradient).all() for gradient in parameter_gradients
    )
